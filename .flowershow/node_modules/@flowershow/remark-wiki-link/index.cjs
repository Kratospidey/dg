...
var walkDir = function walkDir(fullPath) {
  return fs__default["default"].statSync(fullPath).isFile() ? fullPath : getAllFilesRecursively(fullPath);
};
var pathJoinPrefix = function pathJoinPrefix(prefix) {
  return function (extraPath) {
    return path__default["default"].join(prefix, extraPath);
  };
};
var getAllFilesRecursively = function getAllFilesRecursively(folder) {
  return pipe(fs__default["default"].readdirSync, map(pipe(pathJoinPrefix(folder), walkDir)), flattenArray)(folder);
};
function getFiles(type) {
  var prefixPaths = path__default["default"].join(process.cwd(), type);
